...
// Fetch featured blogs - user's 2 latest + 1 from community - cached per request
const getFeaturedBlogs = cache(async (currentUserId: string) => {
...
// Featured blogs skeleton for Suspense fallback
function FeaturedBlogsSkeleton() {
  return (
    <div className="mb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="bg-gray-800/40 border border-gray-700/50 rounded-xl p-5 pb-16 animate-pulse">
            <div className="h-6 bg-gray-700 rounded mb-2 w-3/4" />
            <div className="h-4 bg-gray-700 rounded w-full" />
            <div className="h-4 bg-gray-700 rounded w-2/3 mt-2" />
          </div>
        ))}
      </div>
    </div>
...
// User blogs skeleton for Suspense fallback
function UserBlogsSkeleton() {
  return (
    <div className="space-y-3">
      {[1, 2, 3].map((i) => (
        <div key={i} className="bg-gray-800/30 border border-gray-800 rounded-xl p-5 animate-pulse">
...
// Featured blogs component for streaming
async function FeaturedBlogs({ userId }: { userId: string }) {
  const featuredBlogs = await getFeaturedBlogs(userId);
...
// User blogs component for streaming
async function UserBlogs({ userId }: { userId: string }) {
  const blogs = await Blog.find({ authorId: userId }).sort({ updatedAt: -1 }).lean();
...
export default async function DashboardPage() {
  // Parallelize auth() and currentUser() - don't await sequentially
  ...
  // Connect to DB once - child components will use the same connection
  await dbConnect();
  ...
  return (
    <>
      ...
      {/* Featured Blogs Section - Streams independently */}
      <Suspense fallback={<FeaturedBlogsSkeleton />}>
        <FeaturedBlogs userId={userId} />
      </Suspense>
      ...
      {/* User Blogs - Streams independently */}
      <Suspense fallback={<UserBlogsSkeleton />}>
