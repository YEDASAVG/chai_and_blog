...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (initialProfileRef.current) {
      const hasChanges = 
        ...
        profile.bio !== initialProfileRef.current.bio ||
        profile.github !== initialProfileRef.current.github ||
        profile.linkedin !== initialProfileRef.current.linkedin ||
        profile.twitter !== initialProfileRef.current.twitter;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      initialProfileRef.current = profile; // Reset dirty state
      setIsDirty(false);
    } catch {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]" role="status" aria-label="Loading profile">
        <div className="animate-spin motion-reduce:animate-none rounded-full h-8 w-8 border-t-2 border-b-2 border-[#f97316]"></div>
      </div>
    );
  }
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
              <svg aria-hidden="true" data-lingo-skip className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          ...
          {/* Twitter */}
          <div className="relative">
            <label htmlFor="twitter" className="sr-only">X (Twitter) URL</label>
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
              <svg aria-hidden="true" data-lingo-skip className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
