...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { getBlogById, createOrUpdateBlog, deleteBlog: deleteBlogApi, uploadImage } = useApi();
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (saveStatus === "saving" || (title && !isPublishing)) {
        e.preventDefault();
      }
    };
    window.addEventListener("beforeunload", handleBeforeUnload);
    return () => window.removeEventListener("beforeunload", handleBeforeUnload);
  }, [title, saveStatus, isPublishing]);
  ...
  }, [id, title, status, description, tags, showToast, createOrUpdateBlog]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center" role="status" aria-label="Loading blog">
        <div className="animate-pulse flex flex-col items-center gap-4">
          <div className="w-8 h-8 border-2 border-[#f97316] border-t-transparent rounded-full animate-spin motion-reduce:animate-none"></div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="delete-modal-title">
          <div className="bg-[#1a1a1a] rounded-xl shadow-2xl border border-gray-800 p-6 w-full max-w-md mx-4" style={{ overscrollBehavior: "contain" }}>
            <h3 id="delete-modal-title" className="text-xl font-semibold text-white mb-2">Delete Blog</h3>
          ...
          <div className="flex items-center gap-3">
            ...
            {status === "published" && slug && (
              <Link
                ...
                aria-label="View published blog"
              >
                <svg aria-hidden="true" data-lingo-skip className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
